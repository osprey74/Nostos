...
- **【2026-09-14 原因特定・修正適用済み／耐性試験待ち】コールドブート/固着問題**:
  電源を切って入れ直す・USB を抜き挿しすると描画がガラスに反映されなくなる現象。
  **真因は「EPD_VDD レール(IOE1 io3)がウォームリセットで給電維持されコールドサイクル
  されないこと」**と判明。工場は電源ボタン再起動で PM1 が IOE1 ごと失電させ EPD_EN を
  LOW に落とすため固着が解けるが、espflash のウォームリセットや当方 FW の再起動では
  IOE1 給電が維持され固着コントローラ状態が残る（＝「工場イメージ起動だけが回復させる」
  正体。M5GFX ソフトが特別なのではなく電源ボタンの物理電源サイクルが効いていた）。
  → **[panel.rs `begin()`] で起動時に EPD_VDD を LOW(300ms)→HIGH と強制コールドサイクル**
  する修正を適用。ウォーム再書き込みでもパネルが正常表示することを実機確認（2026-09-14）。
  残: 固着を故意誘発→ウォーム再起動での自己回復（工場イメージ不要）を確認する耐性試験。
- **固着時の復旧手順**（唯一の実績ある方法）:
...
- **【2026-09-14 ソース確証・修正適用済み／実機コールドブート試験待ち】**
  ...
  - `ssd1677-otp` の `BOOSTER_SOFT_START_OTP` 最終バイトを工場値 **`0x40`** に変更
    （0x80 は放電済みパネルのチャージポンプ立ち上げに失敗＝コールド固着の主因と推定）。
    OTP-Demo が 0x80 でも動くのは `M5.begin()` で先に M5GFX が 0x40 で電源投入するため。
  - `init_mono` のコマンド順を工場 list0 に一致（温度センサ 0x18 を booster より前へ）。
  - **次回はこの FW を書き込み、電源断→コールドブートで表示が回復するか実機確認する。**
    改善なければ手がかり③（Deep Sleep Mode 1 のまま電源断）と電源レール投入手順を次に検証。
