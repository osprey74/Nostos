...
- **画面固着（描画がガラスに反映されない）時の復旧手順**: e-ink 更新中の電源変動で
  SSD1677 が固着することがある。EPD_RST／SW_RESET／PM1 リセット（SYS_CMD=0xA2）では
  回復しない。確実な復旧は：
  1. 工場イメージ書き込み（`backups/papermono-factory-original-*.bin` を espflash write-bin）
  2. 電源ボタン 2 秒長押し（赤点滅＝ダウンロードモード）→ 短押し → 工場デモ表示
  3. nostos-fw を書き戻す
